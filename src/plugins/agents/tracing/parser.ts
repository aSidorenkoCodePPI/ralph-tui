/**
 * ABOUTME: Parser for extracting subagent lifecycle events from agent JSONL output.
...
/**
 * Parser for extracting subagent lifecycle events from agent JSONL output.
 *
 * Usage:
 * ```typescript
...
export class SubagentTraceParser {
  ...
  /**
   * Process a single JSONL message from agent output.
   ...
   */
  processMessage(message: AgentJsonlMessage): SubagentEvent[] {
   ...
   */
  processMessages(messages: AgentJsonlMessage[]): SubagentEvent[] {
   ...
   */
  private isTaskToolInvocation(message: AgentJsonlMessage): boolean {
   ...
   */
  private isToolResult(message: AgentJsonlMessage): boolean {
    const raw = message.raw;
   ...
   */
  private isErrorMessage(message: AgentJsonlMessage): boolean {
    const raw = message.raw;
    return (
      raw.type === 'error' ||
      message.type === 'error' ||
   ...
   */
  private handleTaskToolSpawn(message: AgentJsonlMessage): SubagentSpawnEvent | null {
    const raw = message.raw;
   ...
   */
  private handleErrorMessage(message: AgentJsonlMessage): SubagentErrorEvent | null {
